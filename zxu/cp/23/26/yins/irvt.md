...
I don't see how and when we would use the last line because for every insert(root, value) the new value will either be less or greater than the initial root so the insert function would be called on the left or right of the node... wait a second I just figured it out as I was typing we need to return root; after the if, if and else if because the value can also be equal to the root->data.
